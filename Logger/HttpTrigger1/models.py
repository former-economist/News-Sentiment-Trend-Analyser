...
from re import U
from sqlalchemy import DATETIME, Column, Date, ForeignKey, Integer, String, Text, Float, DateTime, Table
...
query_queryresults = Table('results_table', Base.metadata,
    Column('query_id', Integer, ForeignKey('query.id')),
    Column('queryresult_id', Integer, ForeignKey('queryresult.id'))
    )
...
class User(Base):
    ...
    saved_article = relationship("QueryResult", secondary=saved_articles, backref="users",  lazy="selectin")
